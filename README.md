...
- **Tools** — defined in `lib/mcp/tools.ts` (logic in `lib/mcp/service.ts`, reusing `lib/data.ts` readers + Prisma). **Full read + write** scope: tasks (list / search / get / create / update / trash / restore / purge), tags (CRUD) and daily to-dos (create / check / subtasks / trash). IDs are generated by Prisma, not by the LLM.
