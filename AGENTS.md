...
- **Lecture** : Server Components via `lib/data.ts` (`getTasks`, `getTrashedTasks`, `getTags`, `getAllDailyTodos`). `toTask()` y convertit les lignes Prisma vers les types UI (`null → undefined`, `Date → ISO`). Le `layout` et les pages sont `async`.
...
- **État client** : providers montés dans le `layout` (ordre : `ToastProvider` › `SyncProvider` › `ConfirmProvider` › `TagsProvider` › `TasksProvider` › `DailyTodosProvider`), les trois stores de données étant **initialisés depuis la BDD** :
  ...
  - `DailyTodosProvider` (`lib/daily-todos-context.tsx`, `useDailyTodos()`) — todos journalières + sous-tâches ; `/daily` le consomme. Même pattern optimiste que `TasksProvider`.
  - `ToastProvider` (`lib/toast-context.tsx`, `useToast()`) — notifications.
  - `SyncProvider` (`lib/sync-context.tsx`, `useSync()`) — statut de persistance (`idle | syncing | saved | error`), affiché par `sync-indicator.tsx`.
  - `ConfirmProvider` (`lib/confirm-context.tsx`, `useConfirm()`) — dialogue de confirmation in-app (**pas** de `window.confirm` natif).
- **UI optimiste + rollback** : chaque mutation s'applique localement, puis persiste (via `useSync`) ; en cas d'échec → restauration de l'état + **toast**.
...
- **Config** : page `/settings/mcp` (URL, token, snippet `mcp-remote` prêt à coller) avec diode d'état et panneau d'activité (`status.tsx`).
- **Santé / stats** : endpoint non authentifié `app/api/mcp/health/route.ts` — expose `ok`, `tokenConfigured` et des compteurs agrégés (`lib/mcp/stats.ts`, alimentés par `recordMcpEvent`). Ne révèle rien de la BDD ; consommé par le composant de statut de `/settings/mcp`.
